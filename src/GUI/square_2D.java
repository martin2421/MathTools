...
public class square_2D extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        panel_Drawing = new javax.swing.JPanel(){
            public void paintComponent(Graphics g){
                ...
                g2d.fillRect(200, 10, 400, 400);

                g2d.setColor(Color.BLACK);
                g2d.setStroke(new BasicStroke(2));
                g2d.drawRect(200, 10, 400, 400);

                g2d.setFont(new Font("Comic Sans", Font.BOLD, 25));
                g2d.setColor(Color.BLACK);
                g2d.drawString("b", 610, 200);
            }
        };
        button_CLEAR = new javax.swing.JButton();

        setDefaultCloseOperation(javax.swing.WindowConstants.EXIT_ON_CLOSE);
        setTitle("Math Tools - Square (Perimeter and Area)");
        setName("frame_square"); // NOI18N
        setResizable(false);
    ...
    private void button_CALCULATEActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_button_CALCULATEActionPerformed
        ...
        // Tries to parseDouble the corresponding String
        // If it fails: 
        //      if String is empty => add to ArrayList
        //      else String has a character (not a number) => error
        try {
        ...
        // Condition will only enter if there is no error
        if (!(error)) {
            if (emptyTextFields.size() == 3) { // If all inputs are empty => error
                Methods.setemptyTf(input_b);
                Methods.setemptyTf(input_A);
                Methods.setemptyTf(input_P);
            } else if (emptyTextFields.size() == 2) { // If 2 inputs are empty, add them to the ArrayList
                JTextField tempField = emptyTextFields.get(0);
                JTextField tempField2 = emptyTextFields.get(1);
                
                // Depending on which 2 inputs are empty, the program will know what the user wants to calculate
                // i.e: if Area (A) and Perimeter (P) are empty, then use b to calculate them
                if (tempField == input_A && tempField2 == input_P) { // Calculate A and P using b
                    A = Math.pow(b, 2);
                    input_A.setText(String.valueOf(A));
                    P = 4 * b;
                    input_P.setText(String.valueOf(P));
                } else if (tempField == input_A && tempField2 == input_b) { // Calculate A and B using P
                    b = P / 4;
                    input_b.setText(String.valueOf(b));
                    A = Math.pow(b, 2);
                    input_A.setText(String.valueOf(A));
                } else if (tempField == input_P && tempField2 == input_b) { // Calculate P and b using A
                    b = Math.sqrt(A);
                    input_b.setText(String.valueOf(b));
                    P = 4 * b;
                    input_P.setText(String.valueOf(P));
